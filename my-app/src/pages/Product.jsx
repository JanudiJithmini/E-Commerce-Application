      ...
      {/* Product Data */}
        <div className="flex gap-12 sm:gap:12 flex-col sm:flex-row">
            ...
            <div className="flex-1 ">
                ...
                <p className="text-xl font-bold mt-4">{curreny}{productData.price}</p>
                <p className="text-sm text-gray-700 mt-4">{productData.description}</p>
                <div className="flex flex-col gap-4 my-8">
                    <p> Select Size</p>
                    <div className="flex gap-3">
                        {productData.sizes.map((item, index) => (
                            <button onClick={() => setSize(item)} key={index} className={`border cursor-pointer py-2 px-4 bg-gray-100 ${size === item ? 'border-orange-500' : 'border-gray-300'}`} >
                                {item}
                            </button>
                        ))}
                    </div>
     ...
     {/* Description & Rewiew Section */}
      <div className="mt-20">
        ...
        <div className="flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500 ">
            ...
            <p> E-commerce WebSite Loeram ispsum dolor sit amet consectetur adipisicing elit. </p>
        </div>
      </div>
